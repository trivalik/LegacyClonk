...
uint32_t GenerateRandomPlayerColor(int32_t iTry) // generate a random player color for the iTry'th try
{
	...
	return RGB((std::min)(SafeRandom(302), 256), (std::min)(SafeRandom(302), 256), (std::min)(SafeRandom(302), 256));
}
...
void C4PlayerInfoListAttributeConflictResolver::ResolveInInfo()
{
	...
	for (;;)
	{
		...
		else // if (eAttr == PLRATT_Name)
		{
			...
			pResolveInfo->SetForcedName(FormatString("%s (%d)", pResolveInfo->GetOriginalName(), iTries + 1).c_str());
		}
	}
}
