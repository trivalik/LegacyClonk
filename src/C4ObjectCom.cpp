...
bool ObjectComDig(C4Object *cObj) // by DFA_WALK
{
	...
	{
		GameMsgObject(FormatString(LoadResStr("IDS_OBJ_NODIG"), cObj->GetName()).c_str(), cObj);
		return false;
	}
	return true;
}
...
bool ObjectComLineConstruction(C4Object *cObj)
{
	...
	{
		GameMsgObject(FormatString(LoadResStr("IDS_OBJ_NOLINECONSTRUCT"), cObj->GetName()).c_str(), cObj); return false;
	}
	...
	if (!(linekit = cObj->Contents.Find(C4ID_Linekit)))
	{
			...
			|| (cline->Action.Target2 && (cline->Action.Target2->Def->id == C4ID_Linekit)))
		{
			...
			GameMsgObject(FormatString(LoadResStr("IDS_OBJ_NODOUBLEKIT"), cline->GetName()).c_str(), cObj); return false;
		}
		// Create new linekit
		...
		// Message
		GameMsgObject(FormatString(LoadResStr("IDS_OBJ_DISCONNECT"), cline->GetName(), tstruct->GetName()).c_str(), tstruct);
		return true;
	}
	...
	if (cline = Game.FindObject(C4ID_None, 0, 0, 0, 0, OCF_All, "Connect", linekit))
	{
		...
		{
			StartSoundEffect("Connect", false, 100, cObj);
			GameMsgObject(FormatString(LoadResStr("IDS_OBJ_LINEREMOVAL"), cline->GetName()).c_str(), tstruct);
			cline->AssignRemoval();
			return true;
		}
		...
		{
			StartSoundEffect("Error", false, 100, cObj);
			GameMsgObject(FormatString(LoadResStr("IDS_OBJ_NOCONNECTTYPE"), cline->GetName(), tstruct->GetName()).c_str(), tstruct); return false;
		}
		...
		GameMsgObject(FormatString(LoadResStr("IDS_OBJ_CONNECT"), cline->GetName(), tstruct->GetName()).c_str(), tstruct);
	...
	StartSoundEffect("Connect", false, 100, cObj);
	GameMsgObject(FormatString(LoadResStr("IDS_OBJ_NEWLINE"), newline->GetName()).c_str(), tstruct);
...
bool ObjectComChop(C4Object *cObj, C4Object *pTarget)
{
	...
	{
		GameMsgObject(FormatString(LoadResStr("IDS_OBJ_NOCHOP"), cObj->GetName()).c_str(), cObj);
		return false;
	}
	if (cObj->GetProcedure() != DFA_WALK) return false;
...
bool SellFromBase(int32_t iPlr, C4Object *pBaseObj, C4ID id, C4Object *pSellObj)
{
	...
	if (Game.Players.Get(pBaseObj->Base)->Eliminated)
	{
		...
		GameMsgPlayer(FormatString(LoadResStr("IDS_PLR_ELIMINATED"), Game.Players.Get(pBaseObj->Base)->GetName()).c_str(), iPlr); return false;
	}
	// Base owner hostile
	if (Hostile(iPlr, pBaseObj->Base))
	{
		StartSoundEffect("Error", false, 100, pBaseObj);
		GameMsgPlayer(FormatString(LoadResStr("IDS_PLR_HOSTILE"), Game.Players.Get(pBaseObj->Base)->GetName()).c_str(), iPlr); return false;
	}
...
bool Buy2Base(int32_t iPlr, C4Object *pBase, C4ID id, bool fShowErrors)
{
	...
	if (Hostile(iPlr, pBase->Base))
	{
		...
		GameMsgPlayer(FormatString(LoadResStr("IDS_PLR_HOSTILE"), Game.Players.Get(pBase->Base)->GetName()).c_str(), iPlr); return false;
	}
	// buy
