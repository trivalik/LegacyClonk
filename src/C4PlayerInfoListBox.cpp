...
void C4PlayerInfoListBox::ListItem::DrawElement(C4FacetEx &cgo)
{
...
C4GUI::ContextMenu *C4PlayerInfoListBox::PlayerListItem::OnContext(C4GUI::Element *pListItem, int32_t iX, int32_t iY)
{
	...
	if (fFreeSavegamePlayer)
	{
		...
		{
			StdStrBuf strTakeOver(LoadResStr("IDS_MSG_TAKEOVERPLR"));
	...
	else
	{
		...
		if (Game.Network.isHost() || IsLocalClientPlayer())
		{
			...
			{
				StdStrBuf strRemove(LoadResStr("IDS_MSG_REMOVEPLR"));
			...
			{
				StdStrBuf strNewColor(LoadResStr("IDS_MSG_NEWPLRCOLOR"));
...
void C4PlayerInfoListBox::ClientListItem::SetPing(int32_t iToPing)
{
	...
	// get ping as text
	const auto &text = std::to_string(iToPing) + " ms";
	// create ping label if necessary
	if (!pPingLabel)
	{
		pPingLabel = new C4GUI::Label(text.c_str(), GetBounds().Wdt, 0, ARight, C4GUI_MessageFontClr);
		pPingLabel->SetToolTip(LoadResStr("IDS_DLGTIP_PING"));
		AddElement(pPingLabel);
	}
	else
		// or just set updated text
		pPingLabel->SetText(text.c_str());
}
...
C4GUI::Icons C4PlayerInfoListBox::ClientListItem::GetCurrentStatusIcon()
{
	...
	{
		return GetClient()->isLobbyReady() ? C4GUI::Ico_Ready : C4GUI::Ico_Client;
	}
	// observer
...
{
	// safety
	if (!Game.Network.isEnabled()) return nullptr;
...
{
	// safety
	if (!C4GUI::IsGUIValid()) return nullptr;
