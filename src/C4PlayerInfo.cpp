...
bool C4PlayerInfo::LoadFromLocalFile(const char *szFilename)
{
	...
	if (Game.Network.isEnabled() && !Game.C4S.Head.Replay)
	{
		...
		pRes = Game.Network.ResList.getRefRes(Config.AtExeRelativePath(szFilename), true);
		// not found? add
		if (!pRes) pRes = Game.Network.ResList.AddByFile(Config.AtExeRelativePath(szFilename), false, NRT_Player);
...
bool C4PlayerInfo::LoadBigIcon(C4FacetExSurface &fctTarget)
{
	bool fSuccess = false;
...
{
	C4ClientPlayerInfos *pkPlrInfo = nullptr;
