...
bool C4LoaderScreen::Init(const char *szLoaderSpec)
{
	...
	if (!iLoaders)
	{
		...
		{
			LogFatal(FormatString(LoadResStr("IDS_PRC_NOGFXFILE"), C4CFN_Graphics, GfxGrp.GetError()).c_str());
			return false;
		}
		...
		if (!iLoaders)
		{
			LogFatal(FormatString("No loaders found for loader specification: %s/%s/%s/%s", szLoaderSpecPng, szLoaderSpecBmp, szLoaderSpecJpg, szLoaderSpecJpeg).c_str());
			return false;
		}
	}
