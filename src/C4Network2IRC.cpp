...
bool C4Network2IRCClient::OnConn(const C4NetIO::addr_t &AddrPeer, const C4NetIO::addr_t &AddrConnect, const addr_t *pOwnAddr, C4NetIO *pNetIO)
{
	...
	Send("USER", FormatString("clonk x x :%s", RealName.getData()).c_str());
	// Okay
	return true;
}
...
void C4Network2IRCClient::OnDisconn(const C4NetIO::addr_t &AddrPeer, C4NetIO *pNetIO, const char *szReason)
{
	...
	PushMessage(MSG_Status, "", Nick.getData(), FormatString(LoadResStr("IDS_MSG_DISCONNECTEDFROMSERVER"), szReason).c_str());
}
...
{
	if (!Send("QUIT", FormatString(":%s", szReason).c_str()))
		return false;
...
{
	if (!Send("PRIVMSG", FormatString("%s :%s", szTarget, szText).c_str()))
		return false;
...
{
	if (!Send("NOTICE", FormatString("%s :%s", szTarget, szText).c_str()))
		return false;
...
{
	if (!Send("PRIVMSG", FormatString("%s :\1ACTION %s\1", szTarget, szText).c_str()))
		return false;
...
void C4Network2IRCClient::OnCommand(const char *szSender, const char *szCommand, const char *szParameters)
{
	...
	if (SEqualNoCase(szCommand, "JOIN"))
	{
		...
		// Myself?
		if (SenderNick == Nick)
			PushMessage(MSG_Status, szSender, Channel.getData(), FormatString(LoadResStr("IDS_MSG_YOUHAVEJOINEDCHANNEL"), Channel.getData()).c_str());
		else
			PushMessage(MSG_Status, szSender, Channel.getData(), FormatString(LoadResStr("IDS_MSG_HASJOINEDTHECHANNEL"), SenderNick.getData()).c_str());
	}
	// Channel part?
	if (SEqualNoCase(szCommand, "PART"))
	{
		...
		{
			DeleteChannel(pChan);
			PushMessage(MSG_Status, szSender, Nick.getData(), FormatString(LoadResStr("IDS_MSG_YOUHAVELEFTCHANNEL"), Channel.getData(), Comment.getData()).c_str());
		}
		else
			PushMessage(MSG_Status, szSender, Channel.getData(), FormatString(LoadResStr("IDS_MSG_HASLEFTTHECHANNEL"), SenderNick.getData(), Comment.getData()).c_str());
	}
	// Kick?
	if (SEqualNoCase(szCommand, "KICK"))
	{
		...
		{
			DeleteChannel(pChan);
			PushMessage(MSG_Status, szSender, Nick.getData(), FormatString(LoadResStr("IDS_MSG_YOUWEREKICKEDFROMCHANNEL"), Channel.getData(), Comment.getData()).c_str());
		}
		else
			PushMessage(MSG_Status, szSender, Channel.getData(), FormatString(LoadResStr("IDS_MSG_WASKICKEDFROMTHECHANNEL"), Kicked.getData(), Comment.getData()).c_str());
	}
	// Quit?
	if (SEqualNoCase(szCommand, "QUIT"))
	{
		...
		const auto Message = FormatString(LoadResStr("IDS_MSG_HASDISCONNECTED"), SenderNick.getData(), Comment.getData());
	...
	if (SEqualNoCase(szCommand, "TOPIC"))
	{
		...
		// Message
		PushMessage(MSG_Status, szSender, Channel.getData(), FormatString(LoadResStr("IDS_MSG_CHANGESTHETOPICTO"), SenderNick.getData(), Topic.getData()).c_str());
	}
	// Mode?
	if (SEqualNoCase(szCommand, "MODE"))
	{
		...
		// Show Message
		PushMessage(MSG_Status, szSender, Channel.getData(), FormatString(LoadResStr("IDS_MSG_SETSMODE"), SenderNick.getData(), Flags.getData(), What.getData()).c_str());
	}
	// Error?
	...
	if (SEqualNoCase(szCommand, "NICK"))
	{
		...
		// Format status message
		const auto Message = FormatString(LoadResStr("IDS_MSG_ISNOWKNOWNAS"), SenderNick.getData(), NewNick.getData());
...
void C4Network2IRCClient::OnNumericCommand(const char *szSender, int iCommand, const char *szParameters)
{
	...
	case 332: // Topic notify / change
	{
		...
		// Log
		if (Topic.getLength())
			PushMessage(MSG_Status, szSender, Channel.getData(), FormatString(LoadResStr("IDS_MSG_TOPICIN"), Channel.getData(), Topic.getData()).c_str());
	}
	break;
