	...
	if (iRows * iCols)
	{
		...
		for (int32_t iRow = 0; iRow < iRows; ++iRow)
			for (int32_t iCol = 0; iCol < iCols; ++iCol)
			{
				...
				pComp->Value(mkNamingAdapt(pEnt->Text, FormatString("Cell%i_%iString", iCol, iRow).c_str()));
				pComp->Value(mkNamingAdapt(pEnt->iVal, FormatString("Cell%i_%iValue",  iCol, iRow).c_str()));
			}
...
void C4ScoreboardDlg::Update()
{
	...
	for (int32_t iCol = 0; iCol < iColCount; ++iCol)
	{
		...
		for (int32_t iRow = 0; iRow < iRowCount; ++iRow)
		{
			...
			if ((iRow || iCol) && !pCell->Text.isNull()) piColWidths[iCol] = std::max<int32_t>(piColWidths[iCol], Game.GraphicsResource.FontRegular.GetTextWidth(pCell->Text.getData()) + XIndent);
		}
	...
	if (szTitle) iWdt = std::max<int32_t>(iWdt, Game.GraphicsResource.FontRegular.GetTextWidth(szTitle) + 40);
...
void C4ScoreboardDlg::Draw(C4FacetEx &cgo)
{
...
void C4ScoreboardDlg::DrawElement(C4FacetEx &cgo)
{
	typedef C4GUI::Dialog ParentClass;
	ParentClass::DrawElement(cgo);
	// draw spreadsheet
	int32_t iRowCount = pBrd->iRows; int32_t iColCount = pBrd->iCols;
	int32_t iY = YMargin + cgo.TargetY + rcClientRect.y;
	for (int32_t iRow = 0; iRow < iRowCount; ++iRow)
	{
		int32_t iX = XMargin + cgo.TargetX + rcClientRect.x;
		for (int32_t iCol = 0; iCol < iColCount; ++iCol)
		{
